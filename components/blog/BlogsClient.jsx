...
import styles from '@/app/(content)/blogs/Blogs.module.css';

const POSTS_PER_PAGE = 6;

export default function BlogsClient({ initialPosts, categories, totalPosts, config }) {
  const blogQuickRoutes = config.quickRoutes;
  const blogInsights = config.insights;
  const blogCTAItems = config.ctaItems;
  ...
  const featuredPost = showFeatured ? (initialPosts.find((p) => p.featured) || initialPosts[0]) : null;
  ...
  return (
    <>
      <BlogHero
        ...
        stats={[
          { value: `${totalPosts}+`, label: 'In-depth guides' },
          { value: '7 min', label: 'Avg. read time' },
